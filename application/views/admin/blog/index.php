	...
	<section class="wrapper">
	            ...
	            <section class="panel">
		            <header class="panel-heading bg-info">
		                <h1 class="text-infos py-4 pl-3tea py-2">
		                	Blog Information
							<a class="btn btn-default btn-sm pull-right" href="<?=base_url('admin/blogs/create'); ?>"><i class="fa fa-plus"></i> Blog</a>
		                </h1> 
	    ...
	    <script type="text/javascript">
		    ...
		    $(document).ready(function (){
		        ...
		        $('#table').DataTable({
		            ...
		            buttons: [
		                ...
		                {
		                    extend: 'print',
		                    title: "<h3 class='text-center'><?=$this->systemdata->sname; ?></h3><h4 class='text-center'>blog Information</h4><h5 class='text-center'>Printed On <?php echo date('l, d F, Y'); ?></h5>",
		                    exportOptions: {
		                        columns: [1,2,3,4]
		                    },
		        ...
		        //set input/textarea/select event when change value, remove blog error and remove text help block 
			...
			function previewblogImage() {
	            ...
	            oFReader.readAsDataURL(document.getElementById("blogProfile").files[0]);
	            oFReader.onload = function (oFREvent) {
	            document.getElementById("viewblogProfile").src = oFREvent.target.result;
	            };
	        };

	        //previewuserProfileImage
	        function previewblogProfileImage() {
	            var oFReader = new FileReader();
	            oFReader.readAsDataURL(document.getElementById("blogProfile").files[0]);
	            oFReader.onload = function (oFREvent) {
	            document.getElementById("viewblogProfile").src = oFREvent.target.result;
		            ...
		            function (isConfirm) {
		                if (isConfirm) {
		                    $.ajax({
		                        type: "POST",
		                        data: {pod_id: list_id},
		                        url: "<?php echo site_url('admin/blogs/bulk_blogs_delete');?>",
		                        dataType: "JSON",
		                        success: function (data){
		                            if (data.status)
		                            {
		                                swal("blog  Deleted!", "Selected blog Records deleted successfully!", "success");
		                    ...
		                    swal("Cancelled!", "Selected blog Records are NOT deleted!", "error");
		                }
		            });
		        } else
		        {
